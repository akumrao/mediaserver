...
void DummyFrameFilter::go(Frame *frame)
{
       ...
       // SInfo << " Mp4 Wrote: "<<   meta->size << " box name " <<  meta->name << "  muxframe->frametype " <<  muxframe->frametype  << " cam " << cam ;
        ...
        std::string filePath = HRFullPath + "/" + currentTime + "." + boxName;
        ...
        if( muxframe->frametype == 3 )
        {
           ...
           {
               mf.root[dayDate][HR][currentTime] = std::string(currentTime) + "." + boxName;
               mf.save();
           }
        }
