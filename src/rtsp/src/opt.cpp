...
const AVOption *av_opt_next(const void *obj, const AVOption *last)
{
    ...
    if (!obj)
        return NULL;
    class_av = *(const AVClass**)obj;
    if (!last && class_av && class_av->option && class_av->option[0].name)
        return class_av->option;
    if (last && last[1].name)
        return ++last;
    return NULL;
}
...
static int write_number(void *obj, const AVOption *o, void *dst, double num, int den, int64_t intnum)
{
    ...
    switch (o->type) {
    case AV_OPT_TYPE_PIXEL_FMT:
        *(enum AVPixelFormat *)dst = (AVPixelFormat)(llrint(num / den) * intnum);
        break;
    case AV_OPT_TYPE_SAMPLE_FMT:
        *(enum AVSampleFormat *)dst = (AVSampleFormat)(llrint(num / den) * intnum);
        break;
    case AV_OPT_TYPE_BOOL:
    case AV_OPT_TYPE_FLAGS:
    case AV_OPT_TYPE_INT:
...
static int set_string_pixel_fmt(void *obj, const AVOption *o, const char *val, uint8_t *dst)
{
    return set_string_fmt(obj, o, val, dst,
                          AV_PIX_FMT_NB, (int (*)(const char*))av_get_pix_fmt, "pixel format");
}

static int set_string_sample_fmt(void *obj, const AVOption *o, const char *val, uint8_t *dst)
{
    return set_string_fmt(obj, o, val, dst,
                          AV_SAMPLE_FMT_NB, (int (*)(const char*))av_get_sample_fmt, "sample format");
}
...
int av_opt_set(void *obj, const char *name, const char *val, int search_flags)
{
    ...
    switch (o->type) {
    case AV_OPT_TYPE_BOOL:
        return set_string_bool(obj, o, val, (int *)dst);
    ...
    case AV_OPT_TYPE_IMAGE_SIZE:
        return set_string_image_size(obj, o, val, (int*)dst);
    case AV_OPT_TYPE_VIDEO_RATE: {
        AVRational tmp;
    ...
    case AV_OPT_TYPE_PIXEL_FMT:
        return set_string_pixel_fmt(obj, o, val, (uint8_t *)dst);
    case AV_OPT_TYPE_SAMPLE_FMT:
        return set_string_sample_fmt(obj, o, val, (uint8_t *)dst);
    case AV_OPT_TYPE_DURATION:
        if (!val) {
            *(int64_t *)dst = 0;
            return 0;
        } else {
            if ((ret = av_parse_time((int64_t *)dst, val, 1)) < 0)
                av_log(obj, AV_LOG_ERROR, "Unable to parse option value \"%s\" as duration\n", val);
            return ret;
        }
        break;
    case AV_OPT_TYPE_COLOR:
        return set_string_color(obj, o, val, (uint8_t*)dst);
    case AV_OPT_TYPE_CHANNEL_LAYOUT:
        if (!val || !strcmp(val, "none")) {
            *(int64_t *)dst = 0;
        } else {
...
int av_opt_set_bin(void *obj, const char *name, const uint8_t *val, int len, int search_flags)
{
    ...
    ptr = (uint8_t*)(len ? av_malloc(len) : NULL);
    if (len && !ptr)
        return AVERROR(ENOMEM);
...
int av_opt_get(void *obj, const char *name, int search_flags, uint8_t **out_val)
{
    ...
    case AV_OPT_TYPE_STRING:
        if (*(uint8_t **)dst) {
            *out_val = (uint8_t*)av_strdup(*(const char**)dst);
    ...
    case AV_OPT_TYPE_DURATION:
        ...
        ret = strlen((char*)buf); // no overflow possible, checked by an assert
        break;
    case AV_OPT_TYPE_COLOR:
    ...
    while ((opt = av_opt_next(obj, opt))) {
        ...
        if (opt->type != AV_OPT_TYPE_CONST  &&
            ...
            case AV_OPT_TYPE_RATIONAL: {
                ...
                break;
            case AV_OPT_TYPE_PIXEL_FMT:
                av_log(av_log_obj, AV_LOG_INFO, "%s", (char *)av_x_if_null(av_get_pix_fmt_name((AVPixelFormat)opt->default_val.i64), "none"));
                break;
            case AV_OPT_TYPE_SAMPLE_FMT:
                av_log(av_log_obj, AV_LOG_INFO, "%s", (char *)av_x_if_null(av_get_sample_fmt_name((AVSampleFormat)opt->default_val.i64), "none"));
                break;
            case AV_OPT_TYPE_COLOR:
            case AV_OPT_TYPE_IMAGE_SIZE:
            case AV_OPT_TYPE_STRING:
...
void av_opt_set_defaults2(void *s, int mask, int flags)
{
    ...
    while ((opt = av_opt_next(s, opt))) {
        ...
        switch (opt->type) {
            ...
            break;
            case AV_OPT_TYPE_COLOR:
                set_string_color(s, opt, opt->default_val.str, (uint8_t *)dst);
                break;
            case AV_OPT_TYPE_STRING:
                set_string(s, opt, opt->default_val.str, (uint8_t **)dst);
                break;
            case AV_OPT_TYPE_IMAGE_SIZE:
                set_string_image_size(s, opt, opt->default_val.str, (int *)dst);
                break;
            case AV_OPT_TYPE_VIDEO_RATE:
                set_string_video_rate(s, opt, opt->default_val.str, (AVRational *)dst);
                break;
            case AV_OPT_TYPE_BINARY:
...
static int get_key(const char **ropts, const char *delim, char **rkey)
{
    ...
    if (!*opts || !strchr(delim, *opts))
        return AVERROR(EINVAL);
    opts++;
    if (!(*rkey = (char*)av_malloc(key_end - key_start + 1)))
        return AVERROR(ENOMEM);
...
void *av_opt_ptr(const AVClass *class_av, void *obj, const char *name)
{
    const AVOption *opt= av_opt_find2(&class_av, name, NULL, 0, AV_OPT_SEARCH_FAKE_OBJ, NULL);
    if(!opt)
        return NULL;
    ...
    while ((o = av_opt_next(src, o))) {
        ...
        if (o->type == AV_OPT_TYPE_STRING) {
            ...
            *field_dst8 = (uint8_t*)av_strdup((const char*)*field_src8);
            ...
            if (*field_dst8 != *field_src8)
                av_freep(field_dst8);
            *field_dst8 = (uint8_t*)av_memdup(*field_src8, len);
            if (len && !*field_dst8) {
                ret = AVERROR(ENOMEM);
                len = 0;
            }
...
{
    AVOptionRanges *ranges = (AVOptionRanges *)av_mallocz(sizeof(*ranges));
    AVOptionRange **range_array = (AVOptionRange **)av_mallocz(sizeof(void*));
    AVOptionRange *range = (AVOptionRange *)av_mallocz(sizeof(*range));
