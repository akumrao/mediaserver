...
static int default_lockmgr_cb(void **arg, enum AVLockOp op)
{
    ...
    case AV_LOCK_OBTAIN:
        if (!*mutex) {
            pthread_mutex_t *tmp = (pthread_mutex_t*) av_malloc(sizeof(pthread_mutex_t));
            if (!tmp)
        ...
        if ((err = pthread_mutex_lock( (pthread_mutex_t*)*mutex)))
            return AVERROR(err);

        return 0;
    case AV_LOCK_RELEASE:
        if ((err = pthread_mutex_unlock((pthread_mutex_t*)*mutex)))
            return AVERROR(err);
...
{
    if (pos1 <= AVCHROMA_LOC_UNSPECIFIED || pos1 >= AVCHROMA_LOC_NB)
                             ...
                             int buf_size, int align)
{
    ...
    if (planar && nb_channels > AV_NUM_DATA_POINTERS) {
        if (!(frame->extended_data = (uint8_t**)av_mallocz_array(nb_channels,
...
static int audio_get_buffer(AVCodecContext *avctx, AVFrame *frame)
{
    ...
    if (planes > AV_NUM_DATA_POINTERS) {
        frame->extended_data = (uint8_t**) av_mallocz_array(planes, sizeof(*frame->extended_data));
        frame->nb_extended_buf = planes - AV_NUM_DATA_POINTERS;
        frame->extended_buf  = (AVBufferRef**) av_mallocz_array(frame->nb_extended_buf,
...
int attribute_align_arg avcodec_open2(AVCodecContext *avctx, const AVCodec *codec, AVDictionary **options)
{
    ...
    avctx->internal = (AVCodecInternal*)av_mallocz(sizeof(*avctx->internal));
    if (!avctx->internal) {
        ret = AVERROR(ENOMEM);
        goto end;
    }

    avctx->internal->pool = (FramePool*) av_mallocz(sizeof(*avctx->internal->pool));
...
static int unrefcount_frame(AVCodecInternal *avci, AVFrame *frame)
{
    ...
    if (avci->to_free->extended_data != avci->to_free->data) {
        ...
        frame->extended_data = (uint8_t**)av_malloc(size);
...
{
    AVCPBProperties *props = (AVCPBProperties*)av_mallocz(sizeof(AVCPBProperties));
    if (!props)
        return NULL;
...
AVCPBProperties *ff_add_cpb_side_data(AVCodecContext *avctx)
{
    ...
    tmp = (AVPacketSideData*)av_realloc_array(avctx->coded_side_data, avctx->nb_coded_side_data + 1, sizeof(*tmp));
    if (!tmp) {
        av_freep(&props);
        return NULL;
    }
...
{
    AVCodecParameters *par = (AVCodecParameters*)av_mallocz(sizeof(*par));
