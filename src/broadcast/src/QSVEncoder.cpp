...
int QSVEncoder::hwinstance=0;  
QSVEncoder::QSVEncoder(en_EncType encType):QSV_Encoder(encType)
...
//int32_t QSVEncoder::InitEncode(const webrtc::VideoCodec* codec_settings, const webrtc::VideoEncoder::Settings& settings)
//{
//	return 0;
//}
...
void QSVEncoder::ReportError() {
    ...
    SError << "QSVEncoder::codec error" ;
}


 int32_t QSVEncoder::InitEncode(const webrtc::VideoCodec* CodecSetings, int32_t NumberOfCores, size_t MaxPayloadSize)
 {
...
int32_t QSVEncoder::RegisterEncodeCompleteCallback(webrtc::EncodedImageCallback* InCallback)
{
...
void QSVEncoder::SetRates(const RateControlParameters& parameters)
{
  ...
  /*
    if(  codec_.maxFramerate != static_cast<uint32_t>(parameters.framerate_fps) )
    {
        ...
        // In future we might use this code SInfo << "FVideoEncoder::SetRates fps "    <<  codec_.maxFramerate   <<   " bps " << bps; 
    }
   */
...
int32_t QSVEncoder::Release() {
    ...
    SInfo << " QSVEncoder Close Encoder H264";
...
webrtc::VideoFrameType QSVEncoder::ConvertToVideoFrameType(AVFrame *frame) {
...
void QSVEncoder::RtpFragmentize(webrtc::EncodedImage *encoded_image,
...
int32_t QSVEncoder::Encode(const webrtc::VideoFrame& input_frame, const std::vector<webrtc::VideoFrameType>* frame_types)
{
    //SInfo << "Encode";
...
void QSVEncoder::write_frame(AVPacket* enc_pkt)
{
...
//int32_t QSVEncoder::Encode(const webrtc::VideoFrame& frame, const std::vector<webrtc::VideoFrameType>* FrameTypes)
//{
//
//
//
...
// int32_t QSVEncoder::SetChannelParameters(uint32_t PacketLoss, int64 Rtt)
// {
// 	return 0;
// }
