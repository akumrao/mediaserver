 ...
 */
   ...
   */
        static MSRTSPClient* createNew(UsageEnvironment& env, const std::string rtspURL, LiveConnectionContext& ctx, LiveStatus* livestatus, int verbosityLevel = 0, char const* applicationName = NULL, portNumBits tunnelOverHTTPPortNum = 0);

    protected:
        MSRTSPClient(UsageEnvironment& env, const std::string rtspURL, LiveConnectionContext& ctx, LiveStatus* livestatus, int verbosityLevel, char const* applicationName, portNumBits tunnelOverHTTPPortNum);
 ...
 */
   ...
   * 
   */
        static FrameSink* createNew(UsageEnvironment& env, StreamClientState& scs, LiveConnectionContext& ctx , char const* streamId = NULL);

    private:
        FrameSink(UsageEnvironment& env, StreamClientState& scs, LiveConnectionContext& ctx, char const* streamId);
        /** Default virtual destructor */
        virtual ~FrameSink();
