...
{
public:
    H264_Encoder(en_EncType &encType); /* pass in a callback function that is called whenever we decoded a video
                                          frame, make sure to call `readFrame()` repeatedly */
    virtual ~H264_Encoder(); /* d'tor, cleans up the allocated objects and closes the codec context */
    ...
    virtual int init_filters(AVCodecContext *dec_ctx);

    int flags{0};

protected:
    // bool update(bool& needsMoreBytes);                                                     /* internally used
    // to update/parse the data we read from the buffer or file */ int readBuffer(); /* read a bit more data
    // from the buffer */
...
public:
    ...
    // int8_t inbuf[H264_INBUF_SIZE + FF_INPUT_BUFFER_PADDING_SIZE];                         /* used to read
    // chunks from the file */ FILE* fp; /* file pointer to the file from which we read the h264 data */ int
    // frame;                                                                             /* the number of
    // decoded frames */
