...
void echo_write(uv_write_t *req, int status) { SInfo << __func__;
    if (status < 0) {
        fprintf(stderr, "Write error %s\n", uv_err_name(status));
    }
}

void echo_read(uv_stream_t *client, ssize_t nread, const uv_buf_t *buf) { SInfo << __func__;
    if (nread > 0) {
...
void close_process_handle(uv_process_t *req, int64_t exit_status, int term_signal) { SInfo << __func__;
    fprintf(stderr, "Process exited with status %" PRId64 ", signal %d\n", exit_status, term_signal);
    uv_close((uv_handle_t*) req, NULL);
}
...
void on_new_connection(uv_stream_t *server, int status) { SInfo << __func__;
    if (status == -1) {
        // error!
        return;
    }
...
void child_on_new_connection(uv_stream_t *q, ssize_t nread, const uv_buf_t *buf) { SInfo << __func__;
    ...
    uv_pipe_t *pipearvind = (uv_pipe_t*) q;
    if (!uv_pipe_pending_count(pipearvind)) {
        fprintf(stderr, "No pending count\n");
        return;
    }

    uv_handle_type pending = uv_pipe_pending_type(pipearvind);
    assert(pending == UV_TCP);
...
void* new_thread_handle_connection(void* arg) { SInfo << __func__;
    uv_file fd = *(uv_file *) arg;

    loop[1] = (uv_loop_t*) malloc(sizeof (uv_loop_t));
    uv_loop_init(loop[1]);

    uv_pipe_init(loop[1], &pipearvind[1], 1 /* ipc */);
    uv_pipe_open(&pipearvind[1], fd);
    uv_read_start((uv_stream_t*) & pipearvind[1], alloc_buffer, child_on_new_connection);
...
int main()
{
    ...
    uv_pipe_init(loop[0], &pipearvind[0], 1);

    //create a pair of connected sockets
    // if (uv_socketpair(fds,0) != 0)
    //	return -1;

    socketpair(AF_UNIX, SOCK_STREAM, 0, fds);
    uv_pipe_open(&pipearvind[0], fds[0]);
