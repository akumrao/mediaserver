...
static void __last_worker_cleanup(uv_multiplex_worker_t* worker) {
    SInfo << "__last_worker_cleanup " << worker;
...
static void __get_listen_handle(uv_loop_t* loop,
    ...
    }        /* Try again - it might be that the dispatcher wasn't ready */ while (!worker->listener.loop);
...
int uv_multiplex_worker_create(uv_multiplex_t* m,
    ...
    int e = uv_thread_create(&worker->thread, __worker, (void*) worker);
    if (0 != e)
        fatal(e);
    
//    UserInfo* userInf = (UserInfo*)data;
  //  userInf->tid = worker->thread;
