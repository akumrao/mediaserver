...
//    }
    ...
    void awsUdpServer::OnUdpSocketPacketReceived(UdpServer* socket, const char* data, size_t len, struct sockaddr* remoteAddr) {
        ...
        switch (packet.type) {
            ...
            {
                LTrace("First Packet")
                STrace << "Received from " << peerIp << ":" << peerPort <<  " Payload Size:" << packet.payloadlen << " Last Packet NO:" << packet.payload_number;
            ...
            case 1:
            {
                ...
                if (packet.payload_number == curPtr)
                    ...
                    if (packet.payload_number < curPtr) {
                        SError << "Lost Packet found. Sequence No: " << packet.payload_number;
