...
int main(int argc, char **argv)
{
    ...
    std::cout << "press one for 0 x264, 1 vaapi and 2 for qsv" << std::endl;

    std::cin >> nEncoderType;

   H264_Encoder encoder(nEncoderType, NULL, NULL);
...
//   
    ...
    std::string fileName("/tmp/test3.264");
    if (argc > 1) {
      fileName = argv[1];
    }

    encoder.load(fileName, 15, 1280/2 , 720/2);
#elif _WIN32
    int w = 800;
    int h = 600;
