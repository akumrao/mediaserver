...
class H264_Decoder {
public:
    H264_Decoder(st_Cam &cam ); /* pass in a callback function that is called whenever we decoded a video frame, make sure to call `readFrame()` repeatedly */
   ...
   // AVBufferRef *hw_device_ctx{ NULL};
    ...
    bool load(std::string filepath);                                     /* load a video file which is encoded with x264 */
...
protected:
    ...
    enum AVPixelFormat hw_pix_fmt{AV_PIX_FMT_VAAPI};// = AV_PIX_FMT_NONE;
