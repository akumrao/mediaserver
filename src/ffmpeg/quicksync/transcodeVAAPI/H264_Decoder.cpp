...
int H264_Decoder::transcode_write(AVCodecContext *avctx, AVPacket *packet)
{
    ...
    while (1)
    {
        ...
        if (frame->pict_type == AV_PICTURE_TYPE_I)
        {
            if (cam.scale == -1)
            {
                // float fr  = (cdc_ctx->time_base.den
                // )/(cdc_ctx->time_base.num*cdc_ctx->ticks_per_frame); // default
                // SInfo << "frame I";
...
void H264_Decoder::init_hw()
{
    ...
    //    if (pass)
    {
        int ret = av_hwdevice_ctx_create(&avcodec_hw_device_ctx, avcodec_hw_type, NULL, NULL, 0);
...
int H264_Decoder::int_dec()
{
    ...
    //    if (argc < 4) {
    //        fprintf(stderr, "Usage: %s <device type> <input file> <output
    //        file>\n", argv[0]); return -1;
    //    }
    ...
    /* open the input file */
    if (avformat_open_input(&input_ctx, "/var/tmp/test1.264", NULL, NULL) != 0)
    {
    ...
    {
        decoder_ctx->extra_hw_frames = 20;
    }

    decoder_ctx->opaque = decode;

    if (avcodec_hw_type == AV_HWDEVICE_TYPE_VAAPI) { decoder_ctx->get_format = get_vaapi_format; }
    else if (avcodec_hw_type == AV_HWDEVICE_TYPE_QSV)
    {
    ...
    output_file = fopen("/tmp/out.rgb", "w+b");
    ...
    // for(int x = 0; x < 300 ; ++x)
    {
        ...
        if (video_stream == packet.stream_index) ret = transcode_write(decoder_ctx, &packet);
...
H264_Decoder::H264_Decoder(st_Cam &cam)
    : codec(NULL),
      codec_context(NULL),
      parser(NULL),
      fp(NULL),
      ...
      cam(cam)
{
    ...
    if (cam.encType && cam.encType != EN_VP8)
    {
        enum AVHWDeviceType type = AV_HWDEVICE_TYPE_NONE;
        int pass, fail, skip, err;

        if (cam.encType == EN_QUICKSYNC)
            avcodec_hw_type = AV_HWDEVICE_TYPE_QSV;
        else if (cam.encType == EN_VAAPI)
            avcodec_hw_type = AV_HWDEVICE_TYPE_VAAPI;
        else
    ...
    //    if (pass)
    //    {
    //        ret = av_hwdevice_ctx_create(&avcodec_hw_device_ctx,
    //        avcodec_hw_type,
    //                                     NULL, NULL, 0);
    //
    //        if (ret < 0) {
    ...
    //           avcodec_hw_pix_fmt = AV_PIX_FMT_D3D11;
    //         avcodec_sw_pix_fmt = AV_PIX_FMT_NV12;
    //         //AV_PIX_FMT_0RGB32;   // if enabled change the #if(1) to #if(0)
    //         beneath, do not fill the the color
    //         //AV_PIX_FMT_NV12;  //
    //        // AV_PIX_FMT_YUV420P;
    //          //AV_PIX_FMT_NV12;
    //        }
...
{
    decode = new DecodeContext();
    // ffmpeg -decoders

    /*
        VFS..D h264                 H.264 / AVC / MPEG-4 AVC / MPEG-4 part 10
        V....D libopenh264          OpenH264 H.264 / AVC / MPEG-4 AVC / MPEG-4
       part 10 (codec h264) V..... h264_cuvid           Nvidia CUVID H264 decoder
       (codec h264)
     */
    ...
    if (!codec) codec = avcodec_find_decoder_by_name("h264");
...
int H264_Decoder::readBuffer()
{
    ...
    if (bytes_read) { std::copy(inbuf, inbuf + bytes_read, std::back_inserter(buffer)); }
    else
    {
        if (feof(fp))
        {
...
bool H264_Decoder::update(bool &needsMoreBytes)
{
    ...
    while (buffer.size() > 0)
    {
        int len = av_parser_parse2(
            parser, codec_context, &data, &size, &buffer[0], buffer.size(), 0, 0, AV_NOPTS_VALUE);
...
{
    int speed = 1;
    if (cam.speed != 0) { speed = cam.speed; }
    ...
    //        SError <<  "deltamicro > vframecount * vdelay " << deltamicro -
    //        vframecount * vdelay;
    //    }

    while (deltamicro < vframecount * vdelay)
    {
        //        SInfo <<  "deltamicro > vframecount * vdelay " << deltamicro -
        //        vframecount * vdelay;
...
void H264_Decoder::pushQueue(AVFrame *frame)
{
    ...
    {
        if (frameQueue.size() > 0) frameQueue.back().push_front(frame);
    }
