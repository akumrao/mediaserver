    ...
    do {
        ...
        if (got_frame) {
            ...
            while ((ret = av_buffersink_get_frame_flags(is->out_audio_filter, frame, 0)) >= 0) {
                tb = (struct AVRational) av_buffersink_get_time_base(is->out_audio_filter);
#endif
...
static int video_thread(void *arg)
{
    ...
    AVRational frame_rate =  (struct AVRational)av_guess_frame_rate(is->ic, is->video_st, NULL);
