...
ContDetector::process_frame(int64_t pts, int frame_index, char pict_type, const std::vector<AVMotionVector>& motion_vectors) {
    ...
    std::vector<std::vector<cv::Point> > cnts;
        ...
        for( size_t i = 0; i< cnts.size(); i++ )
        {
...
#ifdef AnglePrint
            cv::Mat newMat = cv::Mat(cur->_angular, ret);
            
            float fx = 0;
            float fy = 0; 
            
            cv::Point center_of_rect = (ret.br() + ret.tl())*0.5;

             
            for (int i = ret.y; i <  (ret.y + ret.height); i++) // row 
            {
                for (int j= ret.x ; j < (ret.x + ret.width); j++) // col
                {
                    float ang = cur->_angular.at<float>( i, j );
                    
                    
                    if( ang > -1)
                    {
                        float dx = cur->_mv.at<float>(i, j, 0);
                        float dy = cur->_mv.at<float>(i, j, 1);
                        
                         std::cout <<  "ang: " << ang << "  dx: " <<  dx  << " dy: " << dy  << std::endl;
                        ...
                        if( dx || dy)
                        {
                           ...
                           cur->angle[int (ang/30)] += 1 ;
                        
                    }
                }
            }
           
            float resulForce = sqrtf(fx*fx + fy*fy);
            float direction  = atan2(fy, fx )* 180 / M_PI;
            ...
            std::cout <<  "Force: " << resulForce << " direction: " << direction << std::endl;

            

         //  sort(cur->angle.begin(), cur->angle.end(), greater<int>());

            std::cout << "Angles \n";
            for (auto x : cur->angle)
                std::cout << x << " ";
             std::cout << " End Angles \n" << std::flush;
...
//       }
    ...
    if(mdDetected && cont3MDFrame < 3 )
        ++cont3MDFrame;
    else if(!mdDetected)
        cont3MDFrame = 0;
    
    return (cont3MDFrame > 2 ? true : false);
}
...
_angular(cv::Mat(_rows, _cols, CV_32F, cv::Scalar(-1))),
...
//        }
        ...
        // clipped
        size_t i = std::max(size_t(0), std::min(mv.dst_y / _grid_step, _rows - 1));  //row
        size_t j = std::max(size_t(0), std::min(mv.dst_x / _grid_step, _cols - 1));  //col
