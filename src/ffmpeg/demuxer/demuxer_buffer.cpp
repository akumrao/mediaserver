...
int mediaMuxCallback(void *opaque, uint8_t *buf, int bufSize) {
    ...
    std::cout <<   "h264 bufSize " << bufSize << std::endl << std::flush;
...
//        }
    ...
    std::fstream fileOut("/tmp/tmp.h264", std::ios::out | std::ios::binary);
    if (!fileOut.is_open()) {
        std::cout << "Couldn't open file "  << std::endl;

    }

    fileOut.write((char*) outputData.data(), outputData.size());
    if (fileOut.fail()) {
