...
static int mov_read_covr(MOVContext *c, AVIOContext *pb, int type, int len)
{
    ...
    sc = (MOVStreamContext*) av_mallocz(sizeof(*sc));
    if (!sc)
        return AVERROR(ENOMEM);
    st->priv_data = sc;
...
retry:
    ...
    str = (char*)av_mallocz(str_size_alloc);
    if (!str)
        return AVERROR(ENOMEM);
    ...
    if (parse)
        ...
        if (!raw && (data_type == 3 || (data_type == 0 && (langcode < 0x400 || langcode == 0x7fff)))) { // MAC Encoded
            ...
            int ret = ffio_read_size(pb, (unsigned char*) str, str_size);
            if (ret < 0) {
                av_free(str);
                return ret;
            }
...
static int mov_read_chpl(MOVContext *c, AVIOContext *pb, MOVAtom atom)
{
    ...
    for (i = 0; i < nb_chapters; i++) {
        ...
        ret = ffio_read_size(pb, (unsigned char*) str, str_len);
        if (ret < 0)
            return ret;
        str[str_len] = 0;
...
static int mov_read_dref(MOVContext *c, AVIOContext *pb, MOVAtom atom)
{
    ...
    for (i = 0; i < entries; i++) {
        ...
        if (dref->type == MKTAG('a','l','i','s') && size > 150) {
            ...
            ret = ffio_read_size(pb, (unsigned char*)dref->volume, 27);
            if (ret < 0)
                return ret;
            ...
            len = avio_r8(pb);
            len = FFMIN(len, 63);
            ret = ffio_read_size(pb, (unsigned char*) dref->filename, 63);
            if (ret < 0)
                return ret;
            dref->filename[len] = 0;
            ...
            for (type = 0; type != -1 && avio_tell(pb) < next; ) {
                ...
                if (type == 2) { // absolute path
                    ...
                    ret = ffio_read_size(pb, (unsigned char*)dref->path, len);
                    if (ret < 0) {
                        av_freep(&dref->path);
                        return ret;
                    }
                    ...
                    ret = ffio_read_size(pb, (unsigned char*)dref->dir, len);
                    if (ret < 0) {
                        av_freep(&dref->dir);
                        return ret;
                    }
...
static int mov_read_hdlr(MOVContext *c, AVIOContext *pb, MOVAtom atom)
{
    ...
    if (title_size > 0) {
        ...
        ret = ffio_read_size(pb, (unsigned char*)title_str, title_size);
        if (ret < 0) {
            av_freep(&title_str);
            return ret;
        }
...
static int mov_read_ddts(MOVContext *c, AVIOContext *pb, MOVAtom atom)
{
    ...
    buf = (uint8_t*)av_malloc(ddts_size + AV_INPUT_BUFFER_PADDING_SIZE);
    if (!buf) {
        return AVERROR(ENOMEM);
    }
...
static int mov_read_adrm(MOVContext *c, AVIOContext *pb, MOVAtom atom)
{
    ...
    int i;
    int ret = 0;
    uint8_t *activation_bytes =(uint8_t*) c->activation_bytes;
    uint8_t *fixed_key = (uint8_t*)c->audible_fixed_key;
...
static int mov_read_ftyp(MOVContext *c, AVIOContext *pb, MOVAtom atom)
{
    ...
    ret = ffio_read_size(pb, (unsigned char*) comp_brands_str, comp_brand_size);
    if (ret < 0) {
...
static int mov_read_colr(MOVContext *c, AVIOContext *pb, MOVAtom atom)
{
    ...
    ret = ffio_read_size(pb, (unsigned char*) color_parameter_type, 4);
    if (ret < 0)
        return ret;
    ...
    if (!av_color_primaries_name((AVColorPrimaries)color_primaries))
        color_primaries = AVCOL_PRI_UNSPECIFIED;
    if (!av_color_transfer_name((AVColorTransferCharacteristic)color_trc))
        color_trc = AVCOL_TRC_UNSPECIFIED;
    if (!av_color_space_name((AVColorSpace)color_matrix))
        color_matrix = AVCOL_SPC_UNSPECIFIED;

    st->codecpar->color_primaries = (AVColorPrimaries)color_primaries;
    st->codecpar->color_trc       = (AVColorTransferCharacteristic)color_trc;
    st->codecpar->color_space     = (AVColorSpace)color_matrix;
    av_log(c->fc, AV_LOG_TRACE, "\n");
...
#define ALAC_EXTRADATA_SIZE 36
                st->codecpar->extradata = (uint8_t*)av_mallocz(ALAC_EXTRADATA_SIZE + AV_INPUT_BUFFER_PADDING_SIZE);
...
static int mov_read_stco(MOVContext *c, AVIOContext *pb, MOVAtom atom)
{
    ...
    sc->chunk_count = 0;
    sc->chunk_offsets = (int64_t*)av_malloc_array(entries, sizeof(*sc->chunk_offsets));
    if (!sc->chunk_offsets)
        return AVERROR(ENOMEM);
                                 ...
                                 AVStream *st, MOVStreamContext *sc)
{
    ...
    if (codec_name[0])
        av_dict_set(&st->metadata, "encoder", (const char*)codec_name, 0);
                                 ...
                                 AVStream *st, MOVStreamContext *sc)
{
    ...
    if (sc->format == 0) {
        if (st->codecpar->bits_per_coded_sample == 8)
            st->codecpar->codec_id =(AVCodecID) mov_codec_id(st, MKTAG('r','a','w',' '));
        else if (st->codecpar->bits_per_coded_sample == 16)
            st->codecpar->codec_id = (AVCodecID)mov_codec_id(st, MKTAG('t','w','o','s'));
    }
                                    ...
                                    int64_t size)
{
    ...
    MOVAtom fake_atom = { .type=0, .size = size };
...
static int mov_rewrite_dvd_sub_extradata(AVStream *st)
{
    ...
    st->codecpar->extradata = (uint8_t*)av_mallocz(strlen(buf) + AV_INPUT_BUFFER_PADDING_SIZE);
    if (!st->codecpar->extradata)
        return AVERROR(ENOMEM);
                                ...
                                int64_t size)
{
    ...
    if (st->codecpar->codec_tag == MKTAG('t','m','c','d')) {
        ...
        if (ret < 0)
            return ret;
        if (size > 16) {
            MOVStreamContext *tmcd_ctx = ( MOVStreamContext *)st->priv_data;
            int val;
            ...
            if (size > 30) {
                ...
                if (format == AV_RB32("name") && (int64_t)size >= (int64_t)len + 18) {
                    ...
                    if (str_size > 0 && size >= (int)str_size + 30) {
                        char *reel_name = (char*)av_malloc(str_size + 1);
                        if (!reel_name)
                            return AVERROR(ENOMEM);
...
int ff_mov_read_stsd_entries(MOVContext *c, AVIOContext *pb, int entries)
{
    ...
    for (pseudo_stream_id = 0;
        ...
        id =(AVCodecID) mov_codec_id(st, format);
        ...
        if (sc->extradata) {
            ...
            sc->extradata[pseudo_stream_id] = (uint8_t*)av_malloc(extra_size + AV_INPUT_BUFFER_PADDING_SIZE);
...
static int mov_read_stsd(MOVContext *c, AVIOContext *pb, MOVAtom atom)
{
    ...
    sc->extradata = (uint8_t**)av_mallocz_array(entries, sizeof(*sc->extradata));
    sc->extradata_size = (int*)av_mallocz_array(entries, sizeof(*sc->extradata_size));
    ...
    if (sc->extradata_size[0]) {
        st->codecpar->extradata = (uint8_t*)av_mallocz(sc->extradata_size[0] + AV_INPUT_BUFFER_PADDING_SIZE);
        if (!st->codecpar->extradata)
            return AVERROR(ENOMEM);
...
static int mov_read_stsc(MOVContext *c, AVIOContext *pb, MOVAtom atom)
{
    ...
    sc->stsc_count = 0;
    sc->stsc_data = (MOVStsc*)av_malloc_array(entries, sizeof(*sc->stsc_data));
    if (!sc->stsc_data)
        return AVERROR(ENOMEM);
...
static int mov_read_stps(MOVContext *c, AVIOContext *pb, MOVAtom atom)
{
    ...
    sc->stps_count = 0;
    sc->stps_data = (unsigned int*)av_malloc_array(entries, sizeof(*sc->stps_data));
    if (!sc->stps_data)
        return AVERROR(ENOMEM);
...
static int mov_read_stss(MOVContext *c, AVIOContext *pb, MOVAtom atom)
{
    ...
    sc->keyframe_count = 0;
    sc->keyframes = (int*)av_malloc_array(entries, sizeof(*sc->keyframes));
    if (!sc->keyframes)
        return AVERROR(ENOMEM);
...
static int mov_read_stsz(MOVContext *c, AVIOContext *pb, MOVAtom atom)
{
    ...
    sc->sample_count = 0;
    sc->sample_sizes = (int*)av_malloc_array(entries, sizeof(*sc->sample_sizes));
    if (!sc->sample_sizes)
        return AVERROR(ENOMEM);

    num_bytes = (entries*field_size+4)>>3;

    buf = (unsigned char*)av_malloc(num_bytes+AV_INPUT_BUFFER_PADDING_SIZE);
    if (!buf) {
...
static int mov_read_stts(MOVContext *c, AVIOContext *pb, MOVAtom atom)
{
    ...
    sc->stts_count = 0;
    sc->stts_data = (MOVStts*)av_malloc_array(entries, sizeof(*sc->stts_data));
    if (!sc->stts_data)
        return AVERROR(ENOMEM);
...
static int mov_read_ctts(MOVContext *c, AVIOContext *pb, MOVAtom atom)
{
    ...
    sc->ctts_data = (MOVStts*)av_realloc(NULL, entries * sizeof(*sc->ctts_data));
    if (!sc->ctts_data)
        return AVERROR(ENOMEM);
...
static int mov_read_sbgp(MOVContext *c, AVIOContext *pb, MOVAtom atom)
{
    ...
    sc->rap_group = (MOVSbgp*)av_malloc_array(entries, sizeof(*sc->rap_group));
    if (!sc->rap_group)
        return AVERROR(ENOMEM);
                              ...
                              int count, int duration)
{
    ...
    ctts_buf_new = (MOVStts*)av_fast_realloc(*ctts_data, allocated_size, requested_size);
...
{
    MOVStreamContext *msc = ( MOVStreamContext *)st->priv_data;
    ...
    msc->index_ranges = (MOVIndexRange*)av_malloc((msc->elst_count + 1) * sizeof(msc->index_ranges[0]));
    if (!msc->index_ranges) {
            ...
            if (curr_cts < edit_list_media_time || curr_cts >= (edit_list_duration + edit_list_media_time)) {
                if (st->codecpar->codec_type == AVMEDIA_TYPE_AUDIO && st->codecpar->codec_id != AV_CODEC_ID_VORBIS &&
                    ...
                    if (st->codecpar->codec_type == AVMEDIA_TYPE_AUDIO && edit_list_start_encountered == 0) {
                        num_discarded_begin++;
                        frame_duration_buffer = (int64_t*)av_realloc(frame_duration_buffer,
...
static int mov_read_keys(MOVContext *c, AVIOContext *pb, MOVAtom atom)
{
    ...
    c->meta_keys = (char**)av_mallocz(c->meta_keys_count * sizeof(*c->meta_keys));
    if (!c->meta_keys)
        return AVERROR(ENOMEM);
    ...
    for (i = 1; i <= count; ++i) {
        ...
        c->meta_keys[i] = (char*)av_mallocz(key_size + 1);
        if (!c->meta_keys[i])
            return AVERROR(ENOMEM);
        avio_read(pb, (unsigned char*)c->meta_keys[i], key_size);
    }
...
static int mov_read_custom(MOVContext *c, AVIOContext *pb, MOVAtom atom)
{
    ...
    if (mean && key && val) {
        if (strcmp((const char*)key, "iTunSMPB") == 0) {
            int priming, remainder, samples;
            if(sscanf((const char*)val, "%*X %X %X %X", &priming, &remainder, &samples) == 3){
                if(priming>0 && priming<16384)
                    sc->start_pad = priming;
            }
        }
        if (strcmp((const char*)key, "cdec") != 0) {
            av_dict_set(&c->fc->metadata, (const char*)key, (const char*)val,
...
static int mov_read_tkhd(MOVContext *c, AVIOContext *pb, MOVAtom atom)
{
    ...
    if (!IS_MATRIX_IDENT(res_display_matrix)) {
        ...
        sc->display_matrix = (int32_t*)av_malloc(sizeof(int32_t) * 9);
...
static int mov_read_chap(MOVContext *c, AVIOContext *pb, MOVAtom atom)
{
    ...
    c->chapter_tracks = (int*)new_tracks;
...
static int mov_read_trun(MOVContext *c, AVIOContext *pb, MOVAtom atom)
{
    ...
    if (!sc->ctts_count && sc->sample_count)
    {
        ...
        ctts_data = (MOVStts*)av_realloc(NULL, sizeof(*sc->ctts_data));
        if (!ctts_data)
            return AVERROR(ENOMEM);
...
static int mov_read_sidx(MOVContext *c, AVIOContext *pb, MOVAtom atom)
{
    ...
    index = (MOVFragmentIndex*)av_mallocz(sizeof(MOVFragmentIndex));
    if (!index)
        return AVERROR(ENOMEM);
    ...
    index->items =(MOVFragmentIndexItem*) av_mallocz_array(index->item_count, sizeof(MOVFragmentIndexItem));
    ...
    tmp = (MOVFragmentIndex**)av_realloc_array(c->fragment_index_data,
    ...
    if (offset == avio_size(pb)) {
        for (i = 0; i < c->fc->nb_streams; i++) {
            if (c->fc->streams[i]->id == c->fragment_index_data[0]->track_id) {
                ...
                ref_sc = (MOVStreamContext*)ref_st->priv_data;
                break;
            }
        }
        for (i = 0; i < c->fc->nb_streams; i++) {
...
static int mov_read_elst(MOVContext *c, AVIOContext *pb, MOVAtom atom)
{
    ...
    sc->elst_count = 0;
    sc->elst_data = (MOVElst*)av_malloc_array(edit_count, sizeof(*sc->elst_data));
    if (!sc->elst_data)
        return AVERROR(ENOMEM);
...
{
    int ret = 0;
    uint8_t *buffer = (uint8_t *)av_malloc(len + 1);
    const char *val;
    ...
    if (av_stristr((const char*)buffer, "<GSpherical:StitchingSoftware>") &&
        (val = av_stristr((const char*)buffer, "<GSpherical:Spherical>")) &&
        av_stristr(val, "true") &&
        (val = av_stristr((const char*)buffer, "<GSpherical:Stitched>")) &&
        av_stristr(val, "true") &&
        (val = av_stristr((const char*)buffer, "<GSpherical:ProjectionType>")) &&
...
//            goto out;
        ...
        if (av_stristr((const char*)buffer, "<GSpherical:StereoMode>")) {
            enum AVStereo3DType mode;

            if (av_stristr((const char*)buffer, "left-right"))
                mode = AV_STEREO3D_SIDEBYSIDE;
            else if (av_stristr((const char*)buffer, "top-bottom"))
        ...
        /* orientation */
        val = av_stristr((const char*)buffer, "<GSpherical:InitialViewHeadingDegrees>");
        if (val)
            sc->spherical->yaw = strtol(val, NULL, 10) * (1 << 16);
        val = av_stristr((const char*)buffer, "<GSpherical:InitialViewPitchDegrees>");
        if (val)
            sc->spherical->pitch = strtol(val, NULL, 10) * (1 << 16);
        val = av_stristr((const char*)buffer, "<GSpherical:InitialViewRollDegrees>");
        if (val)
...
static int mov_read_uuid(MOVContext *c, AVIOContext *pb, MOVAtom atom)
{
    ...
    if (!memcmp(uuid, uuid_isml_manifest, sizeof(uuid))) {
        ...
        buffer = (unsigned char*)av_mallocz(len + 1);
        if (!buffer) {
            return AVERROR(ENOMEM);
        }
        ...
        ptr = buffer;
        while ((ptr = (uint8_t *)av_stristr((const char *)ptr, "systemBitrate=\""))) {
            ptr += sizeof("systemBitrate=\"") - 1;
            c->bitrates_count++;
            c->bitrates = (int*)av_realloc_f(c->bitrates, c->bitrates_count, sizeof(*c->bitrates));
            if (!c->bitrates) {
                c->bitrates_count = 0;
                av_free(buffer);
                return AVERROR(ENOMEM);
            ...
            errno = 0;
            ret = strtol((const char*)ptr, &endptr, 10);
        ...
        if (c->export_xmp) {
            ...
            buffer[len] = '\0';
            av_dict_set(&c->fc->metadata, "xmp", (const char*)buffer, 0);
            av_free(buffer);
        } else {
...
static int mov_read_senc(MOVContext *c, AVIOContext *pb, MOVAtom atom)
{
    ...
    sc->cenc.auxiliary_info = (unsigned char*)av_malloc(auxiliary_info_size);
...
static int mov_read_saiz(MOVContext *c, AVIOContext *pb, MOVAtom atom)
{
    ...
    sc->cenc.auxiliary_info_sizes = (unsigned char*)av_malloc(data_size);
        ...
        if (st->codecpar->codec_type == AVMEDIA_TYPE_VIDEO) {
            ...
            for (i = 0; i < st->nb_index_entries; i++) {
                ...
                if (len > sample->size-2)
                    continue;
                title_len = 2*len + 1;
                if (!(title = (unsigned char*)av_mallocz(title_len)))
                    goto finish;
...
static int read_tfra(MOVContext *mov, AVIOContext *f)
{
    ...
    mov->fragment_index_data = (MOVFragmentIndex**)tmp;
    ...
    index->items = (MOVFragmentIndexItem*)av_mallocz_array(
...
{
    MOVContext *mc = (MOVContext *)s->priv_data;
    AVStream *st;
    int sample;
    int i;
...
static const AVOption mov_options[] = {
    ...
    { "activation_bytes", "Secret bytes for Audible AAX files", OFFSET(activation_bytes),
        AV_OPT_TYPE_BINARY, { .i64 = 0 }, 0, 1,.flags = AV_OPT_FLAG_DECODING_PARAM },
    ...
    { "audible_fixed_key", // extracted from libAAX_SDK.so and AAXSDKWin.dll files!
        ...
        AV_OPT_TYPE_BINARY, {.str="77214d4b196a87cd520045fd20a51d67"}, 0, 1,
        .flags = AV_OPT_FLAG_DECODING_PARAM },
    { "decryption_key", "The media decryption key (hex)", OFFSET(decryption_key), AV_OPT_TYPE_BINARY, {.i64 = 0}, 0, 1, .flags = AV_OPT_FLAG_DECODING_PARAM },
