                               ...
                               const char *descr, void *conv)
{
    ...
    case CONV_FUNC_TYPE_INTERLEAVE:
        if (ac->in_fmt == in_fmt && ac->out_fmt == out_fmt &&
            ...
            ac->conv_interleave = ( conv_func_interleave*)conv;
            ac->func_descr      = descr;
            ac->ptr_align       = ptr_align;
            ac->samples_align   = samples_align;
            if (ptr_align == 1 && samples_align == 1) {
                ac->conv_interleave_generic = ( conv_func_interleave*)conv;
                ac->func_descr_generic      = descr;
            } else {
                ac->has_optimized_func = 1;
            }
    ...
    case CONV_FUNC_TYPE_DEINTERLEAVE:
        if (ac->in_fmt == in_fmt && ac->out_fmt == out_fmt &&
            (!channels || ac->channels == channels)) {
            ac->conv_deinterleave = ( conv_func_deinterleave*)conv;
            ac->func_descr        = descr;
            ac->ptr_align         = ptr_align;
            ac->samples_align     = samples_align;
            if (ptr_align == 1 && samples_align == 1) {
                ac->conv_deinterleave_generic = ( conv_func_deinterleave*)conv;
                ac->func_descr_generic        = descr;
            } else {
                ac->has_optimized_func = 1;
            }
...
ff_audio_convert_set_func(ac, ofmt,      ifmt,      0, 1, 1, "C", (void*) CONV_FUNC_NAME(ofmt,      ifmt)); \
ff_audio_convert_set_func(ac, ofmt ## P, ifmt,      0, 1, 1, "C",  (void*)CONV_FUNC_NAME(ofmt ## P, ifmt)); \
ff_audio_convert_set_func(ac, ofmt,      ifmt ## P, 0, 1, 1, "C",  (void*)CONV_FUNC_NAME(ofmt,      ifmt ## P));
                                     ...
                                     int apply_map)
{
    ...
    ac = (AudioConvert*)av_mallocz(sizeof(*ac));
    if (!ac)
        return NULL;
    ...
    // TBD I have removed it since it is not required now. Risky code changes
    //if (ARCH_X86)
      //  ff_audio_convert_init_x86(ac); //arvind
