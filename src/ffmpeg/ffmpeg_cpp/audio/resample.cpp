...
#include "resample_template.ctmp"
...
static int build_filter(ResampleContext *c, double factor)
{
    ...
    tab = (double*)av_malloc(tap_count * sizeof(*tab));
    if (!tab)
...
ResampleContext *ff_audio_resample_init(AVAudioResampleContext *avr)
{
    ...
    c = (ResampleContext*)av_mallocz(sizeof(*c));
    if (!c)
        return NULL;
    ...
    c->filter_bank = (uint8_t*)av_mallocz(c->filter_length * (phase_count + 1) * felem_size);
    if (!c->filter_bank)
        goto error;
