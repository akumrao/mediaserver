                                 ...
                                 int min_samples)
{
    ...
    state->noise_buf = (float*)av_malloc(buf_samples * sizeof(*state->noise_buf));
    if (!state->noise_buf)
    ...
    c->ddsp.dither_int_to_float(state->noise_buf, (int*)noise_buf_ui, nb_samples);
                               ...
                               int channels, int sample_rate, int apply_map)
{
    ...
    c = (DitherContext*)av_mallocz(sizeof(*c));
    if (!c)
        return NULL;
    ...
    c->state = (DitherState*)av_mallocz(channels * sizeof(*c->state));
    if (!c->state)
        goto fail;
    c->channels = channels;
