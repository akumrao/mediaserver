                                   ...
                                   int16_t *block, int n, int qscale)
{
    ...
    if (s->q_scale_type) qscale = (int)ff_mpeg2_non_linear_qscale[qscale];
    else                 qscale <<= 1;
...
static int init_duplicate_context(MpegEncContext *s)
{
    ...
    if (s->avctx->codec_tag == AV_RL32("VCR2")) {
         //typedef int16_t (*pblocks1[12])[64];
       
       // FFSWAP(void *, s->pblocks[4], s->pblocks[5]);  //arvind
        
        int16_t (*tmp)[64] =  s->pblocks[4];
...
int ff_update_duplicate_context(MpegEncContext *dst, MpegEncContext *src)
{
    ...
    if (dst->avctx->codec_tag == AV_RL32("VCR2")) {
        // exchange uv
       // FFSWAP( void * , dst->pblocks[4], dst->pblocks[5]); //arvind
        
        int16_t (*tmp)[64] =  dst->pblocks[4];
...
// {
//     s->y_dc_scale_table      =
//     s->c_dc_scale_table      =  (const uint8_t *)ff_mpeg1_dc_scale_table;
//     s->chroma_qscale_table   = (const uint8_t *)ff_default_chroma_qscale_table;
...
//             if (i) {
//                 s->thread_context[i] =(MpegEncContext*) av_memdup(s, sizeof(MpegEncContext));
...
static void gray_frame(AVFrame *frame)
{
    ...
    av_pix_fmt_get_chroma_sub_sample((AVPixelFormat)frame->format, &h_chroma_shift, &v_chroma_shift);
                         ...
                         int mb_width, int mb_height, int mb_stride, int quarter_sample)
{
    if ((avctx->flags2 & AV_CODEC_FLAG2_EXPORT_MVS) && mbtype_table && motion_val[0]) {
        ...
        {
            #ifdef TRACE
             av_log(avctx, AV_LOG_DEBUG, "%s",  " I frame " );
            #endif
        ...
        AVMotionVector *mvs = (AVMotionVector *)av_malloc_array(mb_width * mb_height, 2 * 4 * sizeof(AVMotionVector));
        if (!mvs)
            return;
    ...
    if ((s->avctx->flags & AV_CODEC_FLAG_PSNR) || s->frame_skip_threshold || s->frame_skip_factor ||
        ...
        if (!s->mb_intra) {
            ...
            if(s->encoding || !(   s->msmpeg4_version || s->codec_id==AV_CODEC_ID_MPEG1VIDEO || s->codec_id==AV_CODEC_ID_MPEG2VIDEO
               ...
               //  ff_wmv2_add_mb(s, block, dest_y, dest_cb, dest_cr); // arvind TBD
                exit(0);
            }
        } else {
