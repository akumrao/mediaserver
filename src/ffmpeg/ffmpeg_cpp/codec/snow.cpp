...
int ff_snow_alloc_blocks(SnowContext *s){
    ...
    av_free(s->block);
    s->block= (BlockNode*)av_mallocz_array(w * h,  sizeof(BlockNode) << (s->block_max_depth*2));
    if (!s->block)
    ...
    for(plane_index=0; plane_index < s->nb_planes; plane_index++){
        ...
        for(level=s->spatial_decomposition_count-1; level>=0; level--){
            for(orientation=level ? 1 : 0; orientation<4; orientation++){
                ...
                av_freep(&b->x_coeff);
                b->x_coeff=(x_and_coeff*)av_mallocz_array(((b->width+1) * b->height+1), sizeof(x_and_coeff));
                if (!b->x_coeff)
                    goto fail;
            }
            w= (w+1)>>1;
