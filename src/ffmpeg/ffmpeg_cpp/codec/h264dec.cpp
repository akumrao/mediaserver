...
{
    H264Context *h = (H264Context*)opaque;
...
static int h264_init_context(AVCodecContext *avctx, H264Context *h)
{
    ...
    h->slice_ctx = (H264SliceContext*)av_mallocz_array(h->nb_slice_ctx, sizeof(*h->slice_ctx));
    if (!h->slice_ctx) {
        h->nb_slice_ctx = 0;
        return AVERROR(ENOMEM);
    }
 ...
 */
 av_always_inline int pred_intra_mode(const H264Context *h,
                                            H264SliceContext *sl, int n)
{
 ...
 av_always_inline void write_back_intra_pred_mode(const H264Context *h,
                                                        H264SliceContext *sl)
{
 ...
 av_always_inline void write_back_non_zero_count(const H264Context *h,
                                                       H264SliceContext *sl)
{
 ...
 av_always_inline void write_back_motion_list(const H264Context *h,
                                                    H264SliceContext *sl,
                                                    int b_stride,
                                                    int b_xy, int b8_xy,
                                                    int mb_type, int list)
{
    ...
    int16_t(*mv_dst)[2] = &h->cur_pic.motion_val[list][b_xy];  // MOTION_VECTOR
 ...
 av_always_inline void write_back_motion(const H264Context *h,
                                               H264SliceContext *sl,
                                               int mb_type)
{
 ...
 av_always_inline int get_dct8x8_allowed(const H264Context *h, H264SliceContext *sl)
{
