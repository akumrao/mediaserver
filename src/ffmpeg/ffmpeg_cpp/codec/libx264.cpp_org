...
{
    X264Context *x4 = (X264Context *)ctx->priv_data;
    uint8_t *p;
    ...
    if (frame) {
        ...
        if (x4->a53_cc) {
            ...
            if (ret < 0) {
                ...
                x4->pic.extra_sei.payloads = (x264_sei_payload_t*)av_mallocz(sizeof(x4->pic.extra_sei.payloads[0]));
                if (x4->pic.extra_sei.payloads == NULL) {
                    ...
                    x4->pic.extra_sei.payloads[0].payload = (uint8_t*)sei_data;
...
FF_ENABLE_DEPRECATION_WARNINGS
    ...
    if (avctx->flags & AV_CODEC_FLAG_GLOBAL_HEADER) {
        ...
        avctx->extradata = p = (uint8_t*)av_mallocz(s + AV_INPUT_BUFFER_PADDING_SIZE);
        if (!p)
        ...
        for (i = 0; i < nnal; i++) {
            ...
            if (nal[i].i_type == NAL_SEI) {
                ...
                x4->sei      = (uint8_t*)av_malloc(x4->sei_size);
                if (!x4->sei)
                    return AVERROR(ENOMEM);
...
static const AVCodecDefault x264_defaults[] = {
    ...
    { (const uint8_t*)"bf",               (const uint8_t*)"-1" },
    { (const uint8_t*)"flags2",           (const uint8_t*)"0" },
    { (const uint8_t*)"g",                (const uint8_t*)"-1" },
    { (const uint8_t*)"i_qfactor",        (const uint8_t*)"-1" },
    { (const uint8_t*)"b_qfactor",        (const uint8_t*)"-1" },
    { (const uint8_t*)"qmin",             (const uint8_t*)"-1" },
    { (const uint8_t*)"qmax",             (const uint8_t*)"-1" },
    { (const uint8_t*)"qdiff",            (const uint8_t*)"-1" },
    { (const uint8_t*)"qblur",            (const uint8_t*)"-1" },
    { (const uint8_t*)"qcomp",            (const uint8_t*)"-1" },
//     { "rc_lookahead",     "-1" },
    { (const uint8_t*)"refs",             (const uint8_t*)"-1" },
#if FF_API_PRIVATE_OPT
    { (const uint8_t*)"sc_threshold",     (const uint8_t*)"-1" },
#endif
    { (const uint8_t*)"trellis",          (const uint8_t*)"-1" },
#if FF_API_PRIVATE_OPT
    { (const uint8_t*)"nr",               (const uint8_t*)"-1" },
#endif
    { (const uint8_t*)"me_range",         (const uint8_t*)"-1" },
#if FF_API_MOTION_EST
    { (const uint8_t*)"me_method",        (const uint8_t*)"-1" },
#endif
    { (const uint8_t*)"subq",             (const uint8_t*)"-1" },
#if FF_API_PRIVATE_OPT
    { (const uint8_t*)"b_strategy",       (const uint8_t*)"-1" },
#endif
    { (const uint8_t*)"keyint_min",       (const uint8_t*)"-1" },
#if FF_API_CODER_TYPE
    { (const uint8_t*)"coder",            (const uint8_t*)"-1" },
#endif
    { (const uint8_t*)"cmp",              (const uint8_t*)"-1" },
    { (const uint8_t*)"threads",          (const uint8_t*)AV_STRINGIFY(X264_THREADS_AUTO) },
    { (const uint8_t*)"thread_type",      (const uint8_t*)"0" },
    { (const uint8_t*)"flags",            (const uint8_t*)"+cgop" },
    { (const uint8_t*)"rc_init_occupancy",(const uint8_t*)"-1" },
    { NULL },
};
...
static const AVClass x264_class = {
    ...
    .option     = options,
    .version    = 1, //LIBAVUTIL_VERSION_INT,
};
