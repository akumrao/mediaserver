...
static int alloc_picture(H264Context *h, H264Picture *pic)
{
    ...
    if (CONFIG_GRAY && !h->avctx->hwaccel && h->flags & AV_CODEC_FLAG_GRAY && pic->f->data[2]) {
        ...
        av_pix_fmt_get_chroma_sub_sample((AVPixelFormat)pic->f->format,
...
static int h264_frame_start(H264Context *h)
{
    ...
    pic->f->pict_type = (AVPictureType)h->slice_ctx[0].slice_type;
...
#define HWACCEL_MAX (CONFIG_H264_DXVA2_HWACCEL + \
    ...
    default:
        ...
        return (AVPixelFormat)AVERROR_INVALIDDATA;
    }
...
static int h264_init_ps(H264Context *h, const H264SliceContext *sl, int first_slice)
{
    ...
    if (!h->context_initialized || must_reinit || needs_reinit) {
        ...
        if ((ret = get_pixel_format(h, 1)) < 0)
            return ret;
        h->avctx->pix_fmt = (AVPixelFormat)ret;
                                   ...
                                   const H2645NAL *nal)
{
    ...
    if ((pps->weighted_pred && sl->slice_type_nos == AV_PICTURE_TYPE_P) ||
        ...
        ret = ff_h264_pred_weight_table((GetBitContext*)&sl->gb, sps, (int*)sl->ref_count,
                           ...
                           const H2645NAL *nal)
{
    ...
    if (h->avctx->debug & FF_DEBUG_PICT_INFO) {
        av_log(h->avctx, AV_LOG_DEBUG,
               ...
               av_get_picture_type_char((AVPictureType)sl->slice_type),
...
{
    H264SliceContext *sl = (H264SliceContext*)arg;
    const H264Context *h = sl->h264;
