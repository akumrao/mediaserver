                               ...
                               int is_avc, void *logctx)
{
    ...
    for (i = 0; i < pkt.nb_nals; i++) {
        ...
        switch (nal->type) {
        case H264_NAL_SPS:
            ret = ff_h264_decode_seq_parameter_set(&nal->gb, (AVCodecContext*)logctx, ps, 0);
            if (ret < 0)
                goto fail;
            break;
        case H264_NAL_PPS:
            ret = ff_h264_decode_picture_parameter_set(&nal->gb, (AVCodecContext*)logctx, ps,
                                                       nal->size_bits);
            if (ret < 0)
                goto fail;
            break;
                                   ...
                                   int err_recognition, void *logctx)
{
    ...
    if (ret < 0 && !(err_recognition & AV_EF_EXPLODE)) {
        ...
        escaped_buf = (uint8_t*)av_mallocz(escaped_buf_size);
        if (!escaped_buf)
