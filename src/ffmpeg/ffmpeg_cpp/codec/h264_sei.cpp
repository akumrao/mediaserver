                                 ...
                                 const H264ParamSets *ps, void *logctx)
{
    ...
    if (sps->pic_struct_present_flag) {
        ...
        h->pic_struct = (SEI_PicStructType)get_bits(gb, 4);
        h->ct_type    = 0;
                                         ...
                                         void *logctx, int size)
{
    ...
    user_data[i] = 0;
    e = sscanf((const char*)user_data + 16, "x264 - core %d", &build);
    if (e == 1 && build > 0)
        h->x264_build = build;
    if (e == 1 && build == 1 && !strncmp((const char*)user_data+16, "x264 - core 0000", 16))
        h->x264_build = 67;

    if (strlen((const char*)user_data + 16) > 0)
                                            ...
                                            GetBitContext *gb)
{
    ...
    if (h->present) {
        h->frame_packing_arrangement_type =(SEI_FpaType) get_bits(gb, 7);
