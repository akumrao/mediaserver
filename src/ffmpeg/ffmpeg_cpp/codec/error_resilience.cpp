...
{
    int b_x, b_y;
    int16_t  (*col )[4] = (int16_t (*)[4]) av_malloc_array(stride, h*sizeof( int16_t)*4);
    uint32_t (*dist)[4] = (uint32_t (*)[4])av_malloc_array(stride, h*sizeof(uint32_t)*4);
...
skip_mean_and_median:
        ...
        FFSWAP(__typeof__(blocklist), blocklist, next_blocklist);
    }
}
