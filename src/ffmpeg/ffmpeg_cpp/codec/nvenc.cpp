...
{
    NvencContext *ctx            = (NvencContext *)avctx->priv_data;
...
static int nvenc_check_codec_support(AVCodecContext *avctx)
{
    ...
    ret = p_nvenc->nvEncGetEncodeGUIDCount(ctx->nvencoder, (uint32_t*)&count);

    if (ret != NV_ENC_SUCCESS || !count)
        return AVERROR(ENOSYS);

    guids = (GUID*)av_malloc(count * sizeof(GUID));
    if (!guids)
        return AVERROR(ENOMEM);

    ret = p_nvenc->nvEncGetEncodeGUIDs(ctx->nvencoder, guids, count, (uint32_t*)&count);
    if (ret != NV_ENC_SUCCESS) {
        ret = AVERROR(ENOSYS);
        goto fail;
    }
...
fail3:
    cu_res = dl_fn->cuda_dl->cuCtxPushCurrent((CUcontext *)ctx->cu_context);
    if (cu_res != CUDA_SUCCESS) {
...
static av_cold int nvenc_setup_device(AVCodecContext *avctx)
{
    ...
    if (avctx->pix_fmt == AV_PIX_FMT_CUDA) {
        ...
        device_hwctx = (AVCUDADeviceContext*)frames_ctx->device_ctx->hwctx;
...
static void nvenc_override_rate_control(AVCodecContext *avctx)
{
    ...
    rc->rateControlMode = (NV_ENC_PARAMS_RC_MODE)ctx->rc;
}
...
static av_cold int nvenc_setup_encoder(AVCodecContext *avctx)
{
    ...
    cu_res = dl_fn->cuda_dl->cuCtxPushCurrent((void**)ctx->cu_context);
    if (cu_res != CUDA_SUCCESS) {
        av_log(avctx, AV_LOG_ERROR, "cuCtxPushCurrent failed\n");
        return AVERROR_EXTERNAL;
    }
...
static av_cold int nvenc_setup_surfaces(AVCodecContext *avctx)
{
    ...
    ctx->surfaces = (NvencSurface*)av_mallocz_array(ctx->nb_surfaces, sizeof(*ctx->surfaces));
    if (!ctx->surfaces)
        return AVERROR(ENOMEM);
...
static av_cold int nvenc_setup_extradata(AVCodecContext *avctx)
{
    ...
    avctx->extradata = (uint8_t*)av_mallocz(outSize + AV_INPUT_BUFFER_PADDING_SIZE);
            ...
            NV_ENC_LOCK_INPUT_BUFFER *lock_buffer_params, const AVFrame *frame)
{
    ...
    ret = av_image_fill_pointers(dst_data, (AVPixelFormat)frame->format, nv_surface->height,
                                 (uint8_t*)lock_buffer_params->bufferDataPtr, dst_linesize);
    if (ret < 0)
        return ret;
...
static int nvenc_register_frame(AVCodecContext *avctx, const AVFrame *frame)
{
    ...
    if (ret != NV_ENC_SUCCESS) {
        nvenc_print_error(avctx, (NVENCSTATUS)ret, "Error registering an input resource");
        return AVERROR_UNKNOWN;
    }
...
static int process_output_surface(AVCodecContext *avctx, AVPacket *pkt, NvencSurface *tmpoutsurf)
{
    ...
    slice_offsets = (uint32_t*)av_mallocz(slice_mode_data * sizeof(*slice_offsets));
