...
#define FIX_MV_MBAFF(type, refn, mvn, idx)      \
 ...
 av_always_inline void pred_pskip_motion(const H264Context *const h,
                                               H264SliceContext *sl)
{
    DECLARE_ALIGNED(4, static const int16_t, zeromv)[2] = { 0 };  //// MOTION_VECTOR
    DECLARE_ALIGNED(4, int16_t, mvbuf)[3][2];
    int8_t *ref     = h->cur_pic.ref_index[0];
    int16_t(*mv)[2] = h->cur_pic.motion_val[0];  //MOTION_VECTOR
 ...
 void fill_decode_caches(const H264Context *h, H264SliceContext *sl, int mb_type)
{
    ...
    if (IS_INTER(mb_type) || (IS_DIRECT(mb_type) && sl->direct_spatial_mv_pred)) {
        ...
        for (list = 0; list < sl->list_count; list++) {
            ...
            int16_t(*mv)[2]       = h->cur_pic.motion_val[list];   //// MOTION_VECTOR
            if (!USES_LIST(mb_type, list))
                continue;
