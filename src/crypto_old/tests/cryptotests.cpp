...
int main(int argc, char** argv)
{
    ...
    describe("sha1 hash", []() {
        ...
        engine.update("abc", 3);
        expects(hex::encode(engine.digest()) == "a9993e364706816aba3e25717850c26c9cd0d89d");

        engine.reset();
        engine.update("abcdbcdecdefdefgefghfghighijhijkijkljklmklmnlmnomnopnopq");
        expects(hex::encode(engine.digest()) == "84983e441c3bd26ebaae4aa1f95129e5e54670f1");

        engine.reset();
        for (int i = 0; i < 1000000; ++i)
            engine.update('a');
        expects(hex::encode(engine.digest()) == "34aa973cd4c4daa4f61eeb2bdbad27316534016f");
    });
    ...
    describe("md5 hash", []() {
        ...
        engine.update("");
        expects(hex::encode(engine.digest()) == "d41d8cd98f00b204e9800998ecf8427e");

        engine.reset();
        engine.update("a");
        expects(hex::encode(engine.digest()) == "0cc175b9c0f1b6a831c399e269772661");

        engine.reset();
        engine.update("abc");
        expects(hex::encode(engine.digest()) == "900150983cd24fb0d6963f7d28e17f72");

        engine.reset();
        engine.update("message digest");
        expects(hex::encode(engine.digest()) == "f96b697d7cb7938d525a2f31aaf161d0");

        engine.reset();
        engine.update("abcdefghijklmnopqrstuvwxyz");
        expects(hex::encode(engine.digest()) == "c3fcd3d76192e4007dfb496cca67e13b");

        engine.reset();
        engine.update("ABCDEFGHIJKLMNOPQRSTUVWXYZ");
        engine.update("abcdefghijklmnopqrstuvwxyz0123456789");
        expects(hex::encode(engine.digest()) == "d174ab98d277d9f5a5611c2c9f419d9f");

        engine.reset();
        engine.update("12345678901234567890123456789012345678901234567890123456789012345678901234567890");
        expects(hex::encode(engine.digest()) == "57edf4a22be3c955ac49da2e2107b67a");
    });
    ...
    describe("file checksum", []() {
        ...
        // expects(crypto::checksum("MD5", path) == "57e14d2f24ab34a6eb1de3eb82f02f33");
    });
