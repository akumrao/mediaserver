...
std::vector<ProbeClusterConfig> ProbeController::InitiateExponentialProbing(
  ...
  assertm(network_available_, "network not available");
  assertm(state_ == State::kInit, "state_ must be State::kInit");
  assertm(start_bitrate_bps_ > 0, "start_bitrate_bps_ must be > 0");
