...
void BitrateProber::CreateProbeCluster(int bitrate_bps,
  ...
  assertm(probing_state_ != ProbingState::kDisabled, "probing disabled");
  assertm(bitrate_bps > 0, "bitrate must be > 0");
  ...
  assertm(cluster.pace_info.probe_cluster_min_bytes >= 0, "cluster min bytes must be >= 0");
...
PacedPacketInfo BitrateProber::CurrentCluster() const {
  ...
  assertm(!clusters_.empty(), "clusters is empty");
  assertm(probing_state_ == ProbingState::kActive, "probing not active");
...
void BitrateProber::ProbeSent(int64_t now_ms, size_t bytes) {
  ...
  assertm(probing_state_ == ProbingState::kActive, "probing not active");
  assertm(bytes > 0, "bytes must be > 0");

  if (!clusters_.empty()) {
    ProbeCluster* cluster = &clusters_.front();
    if (cluster->sent_probes == 0) {
      // RTC_DCHECK_EQ(cluster->time_started_ms, -1);
      assertm(cluster->time_started_ms == -1, "cluster started time must not be -1");
...
int64_t BitrateProber::GetNextProbeTime(const ProbeCluster& cluster) {
  ...
  assertm(cluster.pace_info.send_bitrate_bps > 0, "cluster.pace_info.send_bitrate_bps must be > 0");
  assertm(cluster.time_started_ms > 0, "cluster.time_started_ms must be > 0");
