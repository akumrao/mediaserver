...
namespace RTC
{
	  ...
	  : id(id), listener(listener)
	{
		...
		{
                    base::uv::throwError( mediaCodec->mimeType.ToString()+ " codec not supported for " + RTC::RtpParameters::GetTypeString(this->type));
		}

		auto jsonRtpMappingIt = data.find("rtpMapping");

		if (jsonRtpMappingIt == data.end() || !jsonRtpMappingIt->is_object())
			base::uv::throwError("missing rtpMapping");

		auto jsonCodecsIt = jsonRtpMappingIt->find("codecs");

		if (jsonCodecsIt == jsonRtpMappingIt->end() || !jsonCodecsIt->is_array())
			base::uv::throwError("missing rtpMapping.codecs");

		for (auto& codec : *jsonCodecsIt)
		{
			if (!codec.is_object())
				base::uv::throwError("wrong entry in rtpMapping.codecs (not an object)");
			...
			{
				base::uv::throwError("wrong entry in rtpMapping.codecs (missing payloadType)");
			}
			...
			{
				base::uv::throwError("wrong entry in rtpMapping.codecs (missing mappedPayloadType)");
			}
		...
		{
			base::uv::throwError("missing rtpMapping.encodings");
		}
		...
		for (auto& encoding : *jsonEncodingsIt)
		{
			if (!encoding.is_object())
				base::uv::throwError("wrong entry in rtpMapping.encodings");
			...
			{
				base::uv::throwError("wrong entry in rtpMapping.encodings (missing ssrc or rid)");
			}
			...
			{
				base::uv::throwError("wrong entry in rtpMapping.encodings (missing mappedSsrc)");
			}
		...
		{
			base::uv::throwError("rtpParameters.encodings size does not match rtpMapping.encodings size");
		}
	...
	inline bool Producer::MangleRtpPacket(RTC::RtpPacket* packet, RTC::RtpStreamRecv* rtpStream) const
	{
		...
		// Mangle RTP header extensions.
		{
			...
			// Assign sfuserver RTP header extension ids (just those that sfuserver may
