...
namespace Utils
{
	...
	const uint8_t* Crypto::GetHmacShA1(const std::string& key, const uint8_t* data, size_t len)
	{
		...
		assertm(ret == 1, ("OpenSSL HMAC_Init_ex() failed with key " + key).c_str() );

		ret = HMAC_Update(Crypto::hmacSha1Ctx, data, static_cast<int>(len));

		assertm(
		  ret == 1,
		  ( "OpenSSL HMAC_Update() failed with key " +  key).c_str());
		...
		assertm(
		  ret == 1, ("OpenSSL HMAC_Final() failed with key " + key).c_str());
                
		assertm(resultLen == 20, "OpenSSL HMAC_Final()" );
