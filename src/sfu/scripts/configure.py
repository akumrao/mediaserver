...
if __name__ == '__main__':
    ...
    if sys.platform == 'win32':
        args.append(os.path.join(root, 'sfuserver.gyp'))
        common_fn  = os.path.join(root, 'common.gypi')
        # we force vs 2010 over 2008 which would otherwise be the default for gyp.
        if not os.environ.get('GYP_MSVS_VERSION'):
            os.environ['GYP_MSVS_VERSION'] = '2010'
    else:
        args.append(os.path.join(os.path.abspath(root), 'sfuserver.gyp'))
    ...
    if 'asan' in args:
        args.append('-Dsfuserver_asan=true')
        args = filter(lambda arg: arg != 'asan', args)
    else:
        args.append('-Dsfuserver_asan=false')
