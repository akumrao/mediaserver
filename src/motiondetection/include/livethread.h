...
struct LiveConnectionContext {                                                                        
  ...
  LiveConnectionContext(LiveConnectionType ct, std::string address, SlotNumber slot, std::string &cam,                 
                        bool request_tcp, FrameFilter* framefilter, FrameFilter* info, FrameFilter *txt,  FrameFilter *mvExtractor) :                                                   
  connection_type(ct), address(address), slot(slot), cam(cam), framefilter(framefilter),info(info), txt(txt), mvExtractor(mvExtractor), msreconnect(10000),       
