...
public:
   ...
   */
  static MSRTSPClient* createNew(UsageEnvironment& env, const std::string rtspURL, FrameFilter* fragmp4_muxer, FrameFilter *info, FrameFilter *txt,  FrameFilter *mvExtractor, LiveStatus* livestatus, int verbosityLevel = 0, char const* applicationName = NULL, portNumBits tunnelOverHTTPPortNum = 0);
  
protected:
  MSRTSPClient(UsageEnvironment& env, const std::string rtspURL, FrameFilter* fragmp4_muxer, FrameFilter *info, FrameFilter *txt,  FrameFilter *mvExtractor, LiveStatus* livestatus, int verbosityLevel, char const* applicationName, portNumBits tunnelOverHTTPPortNum);
...
public:
   ...
   * 
   */
  static FrameSink* createNew(UsageEnvironment& env, StreamClientState& scs, FrameFilter* fragmp4_muxer, FrameFilter *info, FrameFilter *txt,  FrameFilter *mvExtractor, char const* streamId = NULL);

private:
  FrameSink(UsageEnvironment& env, StreamClientState& scs, FrameFilter* fragmp4_muxer, FrameFilter *info, FrameFilter *txt,  FrameFilter *mvExtractor, char const* streamId);
  /** Default virtual destructor */
  virtual ~FrameSink();
