...
public:
 ...
 public:
     ReadMp4( std::string ip, int port, StreamingResponderFactory1 *factory , bool multithreaded =true);
 ...
 private:
     ...
     struct stParser
     {
        ...
        FFParse  *fileparser {nullptr};
         
        DummyFrameFilter *fragmp4_filter{nullptr};
        
        FileFrameFilter *filemp4_filter{nullptr};
        
        FrameFilter *fragmp4_muxer{nullptr};
        FrameFilter *filemp4_muxer{nullptr};
          
        FrameFilter *info{nullptr};;
        FrameFilter *txt{nullptr};
        FrameFilter *mvExtractor{nullptr};
        fmp4::LiveConnectionContext *ctx{nullptr};;
        int slot{1};    
        
        stParser(ReadMp4 *mp4this,  std::string cam, bool &md),
        stParser(ReadMp4 *mp4this,  std::string cam , std::string file);
        ~stParser();
 ...
 public:
     ...
     void broadcast(const char * data, int size,  FRAMETYPE frameType,  int frametype , std::string &cam  );
...
//     
     ...
     void get_cam( std::string &cam, std::string &value);
     
     void set_cam( std::string &cam,  std::string &key, std::string &value );
     
     void addCamera( std::string &cam,   bool &md);
     
     void delCamera( std::string &cam);

     void deleteAsync(std::string cam);
     
     void forceDelCamera(std::string &cam ,  std::string  reason );
     
     void playclip( std::string &cam, std::string file);
     
     std::mutex mutDelcamera;
     std::stack< std::string > vecDelcamera;
