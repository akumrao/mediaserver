...
MuxFrameFilter::MuxFrameFilter(const char* name, std::string & cam, FrameFilter *next,  FrameFilter *txt, FrameFilter *muxer) :
FrameFilter(name, next),  initialized(false),  av_format_ctx(NULL), avio_ctx(NULL),
avio_ctx_buffer(NULL), txt(txt), missing(0), ccf(0), av_dict(NULL), format_name("mp4"),  extradata_count(0) {
...
//                if(txt)
//                {
//                    mvExtractor.dst_width = width;
//                    mvExtractor.dst_height = height;
//
//                    mvExtractor.fmt_ctx = av_format_context;
//                    mvExtractor.video_dec_ctx= av_codec_context;
//
//                    
//                    mvExtractor.set();
//                }
...
FragMP4MuxFrameFilter::FragMP4MuxFrameFilter(const char* name, std::string & cam, FrameFilter *next, FrameFilter *txt) :
MuxFrameFilter(name, cam, next, txt, this), got_ftyp(false), got_moov(false)  {
