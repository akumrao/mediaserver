...
void RTSPConnection::playStream() {
    if (is_playing) {
       // SDebug << "RTSPConnection : playStream : stream already playing" ;
        ...
        client = MSRTSPClient::createNew(env, ctx.address, fragmp4_muxer, info, txt, mvExtractor , &livestatus);
...
void SDPConnection :: playStream() {
    ...
    while ((scs->subsession = scs->iter->next()) != NULL) 
    {
        ...
        else
        {
            ...
            scs->subsession->sink= FrameSink::createNew(env, *scs, fragmp4_muxer, info , txt, mvExtractor, ctx.address.c_str());
