...
MSRTSPClient* MSRTSPClient::createNew(UsageEnvironment& env, const std::string rtspURL, FrameFilter* fragmp4_muxer, FrameFilter *info, FrameFilter *txt,  FrameFilter *mvExtractor, LiveStatus* livestatus, int verbosityLevel, char const* applicationName, portNumBits tunnelOverHTTPPortNum) {
  return new MSRTSPClient(env, rtspURL, fragmp4_muxer, info, txt, mvExtractor, livestatus, verbosityLevel, applicationName, tunnelOverHTTPPortNum);
}

MSRTSPClient::MSRTSPClient(UsageEnvironment& env, const std::string rtspURL, FrameFilter* fragmp4_muxer, FrameFilter *info, FrameFilter *txt,  FrameFilter *mvExtractor, LiveStatus* livestatus, int verbosityLevel, char const* applicationName, portNumBits tunnelOverHTTPPortNum) : RTSPClient(env, rtspURL.c_str(), verbosityLevel, applicationName, tunnelOverHTTPPortNum, -1), fragmp4_muxer(fragmp4_muxer),info(info), txt(txt), mvExtractor(mvExtractor), livestatus(livestatus), request_multicast(false), request_tcp(false), recv_buffer_size(0), reordering_time(0) {
}
...
void MSRTSPClient::continueAfterSETUP(RTSPClient* rtspClient, int resultCode, char* resultString) {
  ...
  do {
    ...
    FrameFilter *mvExtractor = ((MSRTSPClient*)rtspClient)->mvExtractor;
   ...
   SDebug << ")\n";
    ...
    scs.subsession->sink = FrameSink::createNew(env, scs, fragmp4_muxer, info, txt, mvExtractor, rtspClient->url());
...
FrameSink* FrameSink::createNew(UsageEnvironment& env, StreamClientState& scs, FrameFilter* fragmp4_muxer, FrameFilter *info, FrameFilter *txt, FrameFilter *mvExtractor, char const* streamId) {
  return new FrameSink(env, scs, fragmp4_muxer,info, txt, mvExtractor, streamId);
}

FrameSink::FrameSink(UsageEnvironment& env, StreamClientState& scs,  FrameFilter* fragmp4_muxer, FrameFilter *info, FrameFilter *txt,  FrameFilter *mvExtractor, char const* streamId) : MediaSink(env), scs(scs), fragmp4_muxer(fragmp4_muxer),info(info),txt(txt), mvExtractor(mvExtractor), on(true), fSubsession(*(scs.subsession))

{
  ...
  { // NEW_CODEC_DEV // when adding new codecs, make changes here
      ...
      fpsFromSDP=fSubsession.videoFPS();
 ...
 // basicframe.payload.resize(checkBufferSize(frameSize)); // set correct frame size .. now information about the packet length goes into the filter chain
  ...
  if (basicframe.h264_pars.slice_type == H264SliceType::sps ||  basicframe.h264_pars.slice_type == H264SliceType::pps) //AUD Delimiter
  {
       ...
       {    
           ...
           if(mvExtractor)
           mvExtractor->run(&basicframe); // starts the frame filter chain
