...
BitReader::BitReader(const std::string & pod, ByteOrder order)
{
    init((const char*)(pod.c_str()), pod.size(), order); // copied
}
...
BitWriter::BitWriter(std::string& pod, ByteOrder order)
{
    init( (char*) (pod.c_str()), pod.size(), order); // copied
}
